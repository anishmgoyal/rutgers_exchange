(function( $ ){
     ...
     * pageList: defaults to "items"; defines the property the pager will look for in data passed in from the server which
     *           will then be interpreted as an array of items on the current page
    ...
    pager.prototype.wrapFunctionality = function (settings) {
        ...
        var successFN = (function (settings) {
            return function (data) {
                if (!isNaN(data.hasOwnProperty(settings.pageList)) && data[settings.pageList].length < settings.pageSize) {
